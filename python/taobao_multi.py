...
shop_url = "https://rate.taobao.com/user-rate-UvG8bvF8SMCcW.htm?spm=a1z10.1-c-s.0.0.kiAzBm"
...
def prox(url, headers):
    ...
    if headers:
        ...
        cc = s.get(url, headers=headers)
    return cc
def get_data(shop_url,cookie,tbname):
    TBNAME = tbname
    ...
    for page in range(1, 50):
        ...
        if req.status_code == 200:
            ...
            db = pymysql.connect(host='5843080a34d43.gz.cdb.myqcloud.com', port=5274, user='shoes', passwd='shoes@2016#$%^', db='shoes', charset='utf8',cursorclass=pymysql.cursors.DictCursor)
            ...
            if s["rateListDetail"]:
                for row  in s["rateListDetail"]:
                    ...
                    if date_unix < last_time_unix:
                        logging.info("now time" + date_unix + "super table time" + last_time_unix)
                        break
                    if result:  # 如果不为空
                        ...
                        try:
                            ...
                            logging.error(e)
                            logging.error(sql)
                            db = pymysql.connect(host='5843080a34d43.gz.cdb.myqcloud.com', port=5274, user='shoes',
                                                 passwd='shoes@2016#$%^', db='shoes', charset='utf8',
                                                 cursorclass=pymysql.cursors.DictCursor)
                    else:
                        continue
            else:
...
get_data(shop_url=shop_url,cookie=cookie,tbname=TBNAME)