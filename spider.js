...
function searchAspect(baseUrl, query_string, spuid, brand) {
  let page_number = 1
    ...
    if(retStatus === 'Success') { // 搜索成功
      ...
      if(aspectHistogramContainer) { // 有属性返回的
        ...
        if(usSizes) {
          ...
          // 使用美码进行二次搜索
          searchWithSizes(baseUrl, valueHistogram, name, 1, query_string, spuid, brand)
        }
        ...
        if(euroSizes) {
          ...
          // 使用欧码进行二次搜索
          searchWithSizes(baseUrl, eurovalueHistogram, euroname, 1, query_string, spuid, brand)
        }

        console.log('============================== search ok =====================================')
        console.log('search and insert ok')
      } else {
      }
    }
  })
...
function searchWithSizes(baseUrl, sizes, name, page_number, query_string, spuid, brand) {
  ...
  //brands.map(brand => {
    sizes.map(item => {
      ...
      //let brandValue = brand['@valueName']
      if(sizeValue !== 'Not Specified') {
        url = `${url}&aspectFilter(0).aspectName=${name}&aspectFilter(0).aspectValueName=${sizeValue}`
        //url = `${url}&aspectFilter(1).aspectName=Brand&aspectFilter(1).aspectValueName=${brandValue}`
...
function makeRequest(page_number, on_going, min_price, query_string, fromdate, todate, app_name, spuid, brand) {
  let url = makeURL(page_number, on_going, min_price, query_string, fromdate, todate, app_name)

  searchAspect(url, query_string, spuid, brand)
}

function runSearch(query_string, start, end, min_price, spuid, brand) {
  // 开始页码
  let page_number = 1
  ...
  // 请求数据，写表体
  makeRequest(page_number, on_going, min_price, query_string, fromdate, todate, app_name, spuid, brand)
}

module.exports = runSearch
/*let keyword_ebay = 'Jordan 4 pure money'
//keyword_ebay = 'Jordan 11 low infrared'
